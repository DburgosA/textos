...
def presentar_pregunta(num_pregunta):
    ...
    # Mostrar la respuesta correcta debajo del puntaje
    respuesta_str = 'Verdadero' if correcta else 'Falso'
    if hasattr(label_respuesta, 'config'):
        label_respuesta.config(text=f": {respuesta_str}")
    ...
    def verificar_respuesta(respuesta_usuario):
        ...
        if respuesta_usuario == correcta:
            ...
            messagebox.showinfo("¡Correcto!", f"Puntaje obtenido: +{puntaje}")
        else:
            puntaje = {'Bajo': 0, 'Medio': -2, 'Alto': -6}[nivel]
            messagebox.showinfo("Incorrecto", f"Respuesta correcta: {'Verdadero' if correcta else 'Falso'}. Puntaje: {puntaje}")
...
label_puntaje = tk.Label(ventana, text="Puntaje: 0", font=("Helvetica", 20))
label_puntaje.pack()
label_nota = tk.Label(ventana, text="Nota: 1.00", font=("Helvetica", 20))
label_nota.pack(pady=10)

# Agregar el label para la respuesta correcta debajo del puntaje
label_respuesta = tk.Label(ventana, text="", font=("Helvetica", 18), fg="#e5e5e5")
